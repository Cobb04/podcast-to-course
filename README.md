...
以前它是一个「拿到转写稿以后很能打」的 Skill。现在它长出了产品的前半截：给它一份转写文件，或者直接给一个公开的小宇宙 / 音频链接；它会在 Apple Silicon Mac 上免费本地转写，再产出结构化框架卡、决策清单、面试表达素材、个人成长日志——对于里程碑级别的单集，还能生成一个漂亮的交互式 HTML 课程。

> 一句话：以前只负责做菜，现在连买菜、洗菜、切菜也包了。ASR API 账单是 0 元，电费和风扇声另算。

| 你扔进来 | 它在背后忙什么 | 你拿走什么 |
|---|---|---|
| 小宇宙公开单集链接 | 解析音频、断点续传、本地 ASR、说话人对齐、质量检查 | `transcript.md` + 可审计中间产物 |
| 公网 `.m4a/.mp3` 链接 | 下载、本地转写、尾部幻觉过滤 | `transcript.md` |
| 已有转写稿 | 清洗、结构化、提炼判断 | 学习卡 / 知识资产 / HTML 课程 |
...
还没有转写稿？问题不大。`podcast-to-course/scripts/ingest_podcast.py` 负责把公开链接变成可靠的 `transcript.md`。默认 `--provider auto`：优先使用免费的本地 WhisperKit，未安装时才回退到已配置的通义听悟。

### 为什么不让 ChatGPT Live 坐旁边听两个小时？

因为那很有喜剧效果，但不太像生产系统：网络抖一下，它可能错过嘉宾最值钱的半句话；电脑还得全程播放，谁打个喷嚏都可能成为神秘第三位嘉宾。

正式方案会下载原始音频，支持断点续传，保留词级时间戳和原始产物，并且可以重复运行、检查和复现。Live 旁听方案保留给公司团建。
...
超过 2 小时的音频会使用增量加载和 VAD 切块，不设人为时长上限——长节目不会在 `02:00:01` 被保安请出去。实际速度与上限由 Mac 性能、磁盘和模型决定。默认开启 SpeakerKit 说话人分离。
正文始终来自 WhisperKit 原生词级时间戳，RTTM 只负责说话人归属，避免分离结果把 `LibLib`、`PMF` 等词拆坏。系统还会以物理音频时长校验尾部漂移，并输出机器可读的质量指标。

当前整期实测参考：`02:08:33` 的中文双人访谈，本地 pipeline 用时约 `24:11`（约 `5.3×` 实时速度）。这是一台机器上的实测，不是宇宙常数；你的 Mac、模型和音质都会参与投票。
