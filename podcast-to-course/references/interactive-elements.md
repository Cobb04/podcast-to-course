...
> **Architecture note for Podcast-to-Course (Tier 3):** CSS lives in `podcast-styles.css`, JS lives in `podcast-engine.js`. Both are inlined by `build-self-contained.sh`. When writing module HTML, use ONLY the patterns below — do NOT inline `<style>` or `<script>` tags. The JS engine auto-initializes by scanning class names and `data-*` attributes.

---

## Podcast-Specific Patterns (use these for podcast-to-course Tier 3)

### Quiz (Scenario-Based)
All quizzes must be scenario-based ("You encounter X, what do you do?"), never knowledge-check.

```html
<div class="quiz" data-quiz-type="scenario">
  <p class="quiz-prompt">你是一个 AI PM。你的 agent 产品任务成功率只有 55%。工程师说需要优化 harness。你的第一反应应该是什么？</p>
  <div class="quiz-options" data-correct="1">
    <button class="quiz-option" data-value="0">A. 同意，开始排期 harness 优化</button>
    <button class="quiz-option" data-value="1">B. 先换一个更强的模型试试</button>
    <button class="quiz-option" data-value="0">C. 增加更多 MCP 插件</button>
    <button class="quiz-option" data-value="0">D. 重写 system prompt</button>
  </div>
  <div class="quiz-feedback">
    <span class="quiz-feedback-text" hidden>模型才是 agent。基础成功率不到 60% 时，换更强的模型是性价比最高的第一步。harness 是放大器——信号太弱的话放大也没用。</span>
  </div>
</div>
```

**Rules:**
- `data-correct` on `.quiz-options` must be a plain integer (e.g., `"1"`).
- Feedback text goes in `<span class="quiz-feedback-text" hidden>` — NEVER in a `data-correct` attribute on `.quiz-feedback`. Chinese quotes in attributes break HTML.
- `data-value` on each option must match the `data-correct` value for the correct answer.
- JS handles: click-to-select, disable-after-answer, green-correct / red-incorrect, show-feedback.

### Original Words ↔ Actionable Insight Translation

```html
<div class="quote-translation" data-type="insight-pair">
  <div class="quote-left">
    <span class="quote-speaker">新路</span>
    <blockquote>"模型以外都是 harness。"</blockquote>
  </div>
  <div class="insight-right">
    <span class="insight-label">🔑 可操作洞见</span>
    <p>模型决定智力天花板，harness 决定智力能发挥多少。</p>
    <p class="insight-trigger"><strong>何时使用：</strong>每次评估 agent 产品时，先分清楚问题出在哪一层。</p>
  </div>
</div>
```

### Framework Visualization (Layered Reveal)

```html
<div class="framework-visualization">
  <div class="fw-layer">
    <strong>🔧 第一层：执行能力层</strong>
    <p>CLI工具 · 文件系统 · 浏览器 · 语言解释器</p>
  </div>
  <div class="fw-layer">
    <strong>🧠 第二层：上下文状态层</strong>
    <p>Memory · Skills · 上下文卸载 · Agent接棒</p>
  </div>
  <div class="fw-layer">
    <strong>🏛️ 第三层：编排治理层</strong>
    <p>多Agent协调 · 权限控制 · 沙箱隔离</p>
  </div>
  <button class="fw-btn">展开下一层 →</button>
</div>
```

**Rules:**
- Button must have class `fw-btn`.
- Must have at least 2 `.fw-layer` elements.
- First layer is always visible; JS hides layers 2+ and reveals them on click.
- DO NOT put inline styles on `.fw-layer` elements. CSS handles opacity, background, border.

### Side Annotation (inline callout card)

An inline aside that highlights a key insight, framework, or product takeaway. Renders as a styled card within the normal document flow — never uses absolute positioning.

```html
<aside class="screen-annotation">
  <strong>🧠 思维模型</strong>新路的底层信念：工程应该适配模型。这个比喻是整期播客的"根隐喻"。
</aside>
```

Place inside `.screen-content`, typically before the heading or paragraph it annotates. The `<strong>` element serves as the label (monospace, uppercase, accent-colored). The remaining text is the annotation body.

**Rules:**
- Always in normal document flow. **Never** use `position: absolute`, `right` negative offsets, `top`, or any out-of-flow positioning.
- Renders as a warm-tinted card with left accent border, consistent across all screen widths.
- Max 1-2 lines of text. If you need more, use a `.callout-insight` box instead.
- 1-2 per module max. Only on screens with a key insight to reinforce.

**When to use:** When a screen has a "aha" moment, framework summary, or product judgment that benefits from visual emphasis beyond a regular paragraph.

### Conversation Flow (Chat Group)

```html
<div class="chat-group" id="paradigm-debate">
  <div class="chat-message chat-left">
    <span class="chat-sender">Prompt Flow 派</span>
    <p>我们预设好每个节点的输入输出，这样可控、可调试。</p>
  </div>
  <div class="chat-message chat-right">
    <span class="chat-sender">Agent-Native 派</span>
    <p>你预设的每一步都是在限制模型。模型应该自己决定下一步。</p>
  </div>
</div>
```
...
```html
<div class="growth-log-card">
  <div class="gl-section">
    <span class="gl-label">Previously I thought...</span>
    <p>Agent 产品好不好主要看模型 + prompt。</p>
  </div>
  <div class="gl-arrow">↓</div>
  <div class="gl-section">
    <span class="gl-label">Now I think...</span>
    <p>模型是起点，harness 是放大器。在模型趋同时代，harness 才是差异化来源。</p>
  </div>
</div>
```
...
<div class="callout-insight">
  ...
  <div class="callout-text">
    <strong>核心问题：</strong>如果 agent 的上限是由 harness 决定的，那作为产品人，你必须能判断一个 harness 是好是坏。
  </div>
</div>
```
...
<div class="action-item">
  ...
  <div class="action-body">
    <strong>检查你产品的 agent 工具链</strong>
    <p>用了 MCP 的，这周做一个 CLI vs MCP 的 A/B 测试，对比成功率。</p>
    <span class="credibility-badge">🟡 基于新路的工程经验</span>
  </div>
</div>
```
...
<div class="pattern-cards">
  <div class="pattern-card">
    <span class="pattern-icon">1️⃣</span>
    <h4>模型本身不够强</h4>
    <p>如果基础任务成功率不到 60%，先换模型。Harness 是放大器。</p>
    <span class="credibility-badge">🟢 High | 基本工程常识</span>
  </div>
</div>
```
...
<table class="insight-table">
  <tr><th>你的产品阶段</th><th>重点投入</th></tr>
  <tr><td>单 Agent + 简单任务</td><td><strong>第一层</strong> — 配好 3 类核心工具</td></tr>
</table>
```

---

## Original Codebase-to-Course Patterns (for reference / Tier 3 fallback)
